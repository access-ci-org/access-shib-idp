...
## Obtain a New Amazon AWS TLS Cert for HTTPS

The TLS/SSL connection (`https://` on port 443) is secured by an Amazon-issued
certificate. 
...
In the "Search for services" box at the top of the page, enter
"[Certificate Manager](https://us-east-2.console.aws.amazon.com/acm/home?region=us-east-2)".

In the main "Certificates" window, click the "Request" button.

On the "Request certificate" page, ensure the "Certificate type" has the
"Request a public certificate" radio button checked. Then click the "Next"
button.

On the "Request public certificate" page, enter the following information:

* Domain names, Fully qualified domain name: **idp.access-ci.org**
...
On the next page, in the "Certificate status" section, note the ARN (AWS
Registration Number) for the new idp.access-ci.org certificate. This will be
used as the default value in the
...
In the "Domains" section, you should see Status: Pending validation. Issuing
this certificates requires adding a DNS entry for idp.access-ci.org. So [open
...
Description:
    Please add a new DNS entry for idp.access-ci.org to be used by AWS for
...
connection. However, due to the [news](https://www.sectigo.com/47-day-ssl)
that certificate lifetimes would be drastically cut over the next few years,
an Amazon certificate is now used.  The Amazon certificate is managed by AWS
and updated automatically. Since the certificate is tied to the Load Balancer,
the IdP service does not need to be restarted upon certificate refresh.
